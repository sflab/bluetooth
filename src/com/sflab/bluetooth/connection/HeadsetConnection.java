...
public class HeadsetConnection implements BtConnection {
	...
	public HeadsetConnection(Context context, ServiceListener l) {
		mContext = context;
		mServiceListener = l;
		if (!context.bindService(new Intent(IBluetoothHeadset.class.getName()),
				mConnection, 0)) {
	...
	/**
	 * Extra to be used with the Headset State change intent. This will be used
	 * only when Headset state changes to {@link #STATE_DISCONNECTED} from any
	 * previous state. This extra field is optional and will be used when we
	 * have deterministic information regarding whether the disconnect was
	 * initiated by the remote device or by the local adapter.
	 */
	...
	/**
	 * Default priority for headsets that for which we will accept inconing
	 * connections and auto-connect
	 */
	public static final int PRIORITY_AUTO_CONNECT = 1000;
	/**
	 * Default priority for headsets that for which we will accept inconing
	 * connections but not auto-connect
	 */
	public static final int PRIORITY_ON = 100;
	/**
	 * Default priority for headsets that should not be auto-connected and not
	 ...
	 * BluetoothHeadset will return default error results once close() has been
	 ...
	 * 
	 * @return current headset, or null if not in connected or connecting state,
	 *         or if this proxy object is not connected to the Headset service.
	 */
	...
	/**
	 * Request to initiate a connection to a headset. This call does not block.
	 * Fails if a headset is already connecting or connected. Initiates
	 * auto-connection if device is null. Tries to connect to all devices with
	 * priority greater than PRIORITY_AUTO in descending order.
	 * 
	 * @param device
	 *            device to connect to, or null to auto-connect last connected
	 *            headset
	 * @return false if there was a problem initiating the connection procedure,
	 *         and no further HEADSET_STATE_CHANGED intents will be expected.
	 */
	 ...
	 * connecting). Returns false if not connected, or if this proxy object if
	 ...
	 * be made asynchornous. Returns false if this proxy object is not currently
	 * connected to the Headset service.
	 */
